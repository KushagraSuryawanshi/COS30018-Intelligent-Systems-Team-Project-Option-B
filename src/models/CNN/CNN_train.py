...
"""
CNN Training Script for MNIST classification.

Trains a simple CNN on MNIST data and optionally includes
additional labeled data from external directories.

Classes:
 - SimpleCNN: Basic CNN model architecture for MNIST classification.

Functions:
 - make_extra_transform()
 - get_dataloaders(batch_size, extra_dir)
 - train_main(epochs, batch_size, lr, extra_dir, out_dir)
 - evaluate_final(model, loader, criterion)
 - plot_confusion_matrix(y_true, y_pred, classes, save_path)
 - save_sample_preds_grid(model, loader, n, save_path)
"""
...
def make_extra_transform():
    """
    Create transformation pipeline for external cropped digit images.
...
def get_dataloaders(
    ...
    """
    Create MNIST train, validation, and test dataloaders.

    Optionally includes additional labeled images from an external
    ImageFolder (expects 0–9 subdirectories).

    :param batch_size: Batch size for training and evaluation.
    :param extra_dir: Optional path to additional labeled data.
...
def train_main(
    ...
    """
    Train CNN model on MNIST (optionally with external data).

    :param epochs: Number of epochs to train.
    :param batch_size: Training batch size.
    :param lr: Learning rate.
    :param extra_dir: Optional extra labeled data path.
    :param out_dir: Output directory to save models/logs.
    :return: Path to saved best model.
    """
    out_dir = out_dir or MODELS_DIR
    train_loader, val_loader, test_loader = get_dataloaders(
        batch_size=batch_size, extra_dir=extra_dir
    )
    ...
    test_loss, test_acc, test_preds, test_targets = evaluate_final(
    ...
    try:
        plot_confusion_matrix(
            ...
            save_path=os.path.join(out_dir, "cm_mnist.png")
        )
        save_sample_preds_grid(
            model, test_loader,
            n=25, save_path=os.path.join(out_dir, "sample_preds.png")
        )
...
def plot_confusion_matrix(y_true, y_pred, classes=tuple(range(10)), save_path=None):
    ...
    :param classes: Class labels (default 0–9).
...
def save_sample_preds_grid(model, loader, n=25, save_path=None):
    """
    Save a grid of sample predictions for visualization.
    ...
    """
    model.eval()
    images_batch, _ = next(iter(loader))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Optional: path to labeled extra data (0..9 subfolders)"
    )
