...
"""
Feedforward Neural Network (FNN) training script for MNIST dataset.

Functions:
 - make_extra_transform()
 - get_dataloaders(batch_size=200, extra_dir=None)
 - train_and_save(epochs=10, lr=0.05, batch_size=200, extra_dir=None, ...)
Classes:
 - Net: Defines a 4-layer fully connected neural network for MNIST classification.
"""
...
DATA_DIR = os.path.expanduser("~/.pytorch/mnist")
...
def make_extra_transform():
    """Create transformation pipeline for external cropped image data."""
...
def get_dataloaders(batch_size: int = 200, extra_dir: Optional[str] = None):
    """Prepare PyTorch dataloaders for MNIST training and testing."""
...
def train_and_save(
        ...
        save_name: str = "fnn_net.pt",
        device: Optional[str] = None
):
    """Train the FNN on MNIST data and save the trained model."""
